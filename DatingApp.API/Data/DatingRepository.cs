    ...
    public class DatingRepository : IDatingRepository
    {
        ...
        private async Task<IEnumerable<int>> GetUserLikes(int id,bool likers)
        {
            var user = await _context.Users
                .Include(x => x.Likers).Include(a => a.Likees).FirstOrDefaultAsync(a=>a.Id==id);
            if (likers)
            {
                return user.Likers.Where(u => u.LikeeId == id).Select(a=>a.LikerId);
            }
            else
            {
                return user.Likees.Where(u => u.LikerId == id).Select(a => a.LikeeId);
            }
        }
